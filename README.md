...
- **Roadbed.Data\*** collapses `Roadbed.Data`, `Roadbed.Data.Dapper`, `Roadbed.Data.MySql`, `Roadbed.Data.Sqlite`, and `Roadbed.Data.Postgresql`. The base package is the abstraction; the per-provider packages supply the executor and Dapper handlers.
- **Roadbed.IO\*** collapses `Roadbed.IO` (typed file abstractions) and `Roadbed.IO.Csv` (CSV reading/writing).
- `Roadbed.Crud` and `Roadbed.Logging` are the only consumable-layer packages that sit on `Roadbed.Data\*`; the rest reach `Roadbed.Common` directly.
...
- [Roadbed.Secrets.KeePass](/src/Roadbed.Secrets.KeePass/README.md) - KeePass-backed secret loading
