...
This document is the authoritative reference for the Roadbed.Data.Dapper NuGet package. When a developer asks you to create a data access layer that uses Dapper with SQLite, use this document together with the [Roadbed.Crud Architecture](architecture-roadbed-crud.md), [Roadbed.Data Architecture](architecture-roadbed-data.md), and [Roadbed.Data.Sqlite Architecture](architecture-roadbed-sqlite.md) to scaffold the correct entity types, type handler registration, column mapping configuration, and repository/service patterns.
...
4. [Type Handlers](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#type-handlers)
    - [DateTime Handling](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#datetime-handling)
    - [DateTimeOffset Handling](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#datetimeoffset-handling)
    - [Storage Format Reference](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#storage-format-reference)
    - [Choosing DateTime vs DateTimeOffset](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#choosing-datetime-vs-datetimeoffset)
5. [Column Mapping](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#column-mapping)
    - [DapperMapping.Configure()](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#dappermappingconfigure)
    - [Resolution Order](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#resolution-order)
    - [Entity Example with Column Attributes](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#entity-example-with-column-attributes)
6. [Relationship to Roadbed.Crud](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#relationship-to-roadbedcrud)
    - [Entity Hierarchy](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#entity-hierarchy)
    - [Why BaseEntityClass for Database Entities](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#why-baseentityclass-for-database-entities)
    - [Repository and Service Layer](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#repository-and-service-layer)
7. [Installer Wiring](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#installer-wiring)
    - [Registration Order](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#registration-order)
    - [IEntity Interface Scanning](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#ientity-interface-scanning)
    - [Complete Installer Example](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#complete-installer-example)
8. [Implementation Walkthrough](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#implementation-walkthrough)
9. [Common Pitfalls](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#common-pitfalls)
...
│   IDbFooRepository                 (from Roadbed.Crud)       │
│   DbFooRepository                  (uses SqliteExecutor)     │
│   IDbFooService                    (from Roadbed.Crud)       │
│   DbFooService                     (virtual, calls repo)     │
...
```csharp
namespace Foo.Database.Entities;
...
│                          │
│ Depends on:              │
│   IDbFooService          │  ← Roadbed.Crud service interface (public)
└──────────┬───────────────┘
           │
┌──────────▼───────────────┐
│ DbFooService             │
...
The repository is `internal` — the application layer depends on the service interface, not the repository. See the [Roadbed.Crud Architecture](https://claude.ai/Roadbed.Crud/docs/architecture.md) for full details on the repository/service separation.
...
3. services.AddScoped<IRepo, Repo>()      ← Repository registrations
4. services.AddScoped<IService, Service>() ← Service registrations
5. ServiceLocator.SetLocatorProvider(...)  ← Snapshot for NuGet self-containment
```
...
using Roadbed.Crud;
using Roadbed.Data;
using Foo.Database.Entities;
using Foo.Database.Repositories;
using Foo.Database.Services;
    ...
    : IServiceCollectionInstaller
{
    ...
    public void ConfigureServices(IServiceCollection services, IConfiguration configuration)
    {
        ...
        // Step 4: Register repositories (internal) and services (public)
        services.AddScoped<IDbFooRepository, DbFooRepository>();
        services.AddScoped<IDbFooService, DbFooService>();
        services.AddScoped<IDbBarRepository, DbBarRepository>();
        services.AddScoped<IDbBarService, DbBarService>();
...
```csharp
namespace Foo.Database.Repositories;
...
internal sealed class DbFooRepository : BaseClassWithLogging, IDbFooRepository
{
    ...
    public async Task<DbFoo?> ReadAsync(
        long id,
...
The service delegates to the repository and can add business logic via overrides. See the [Roadbed.Crud Architecture](https://claude.ai/Roadbed.Crud/docs/architecture.md) for details on the repository/service separation.

### Step 5: Wire Up the Installer

Add the Dapper configuration to the installer (see [Complete Installer Example](https://claude.ai/chat/95ea44ba-8bb4-432a-9215-0d25934a8fcc#complete-installer-example) above).
...
// ✅ Better — interface scanning discovers only IEntity implementations
Type[] entityTypes = typeof(DbFoo).Assembly
    .GetTypes()
    .Where(t => t.IsClass &&
                !t.IsAbstract &&
...
// ✅ Correct — Dapper configuration first, then repositories and services
public void ConfigureServices(IServiceCollection services, IConfiguration configuration)
{
    DapperMapping.Configure(entityTypes);
    SqlMapper.AddTypeHandler(new DapperDateTimeHandler());
...
- [ ] Register all repository interfaces → implementations
- [ ] Register all service interfaces → implementations
...
```
1. DapperMapping.Configure(dtoTypes)     ← Column attribute mapping
2. SqlMapper.AddTypeHandler(...)          ← All 4 type handlers
3. services.AddScoped<IRepo, Repo>()     ← Repository registrations
...
- `DapperMapping.Configure()` sets up `SqlMapper.SetTypeMap()` for each DTO type. If a repository executes a query before this runs, Dapper won't know how to map snake_case columns to PascalCase properties.
- Type handlers must be registered before any query that returns `DateTime` or `DateTimeOffset` columns. Without them, Dapper may return incorrect values or throw exceptions.

### Assembly Scanning for DTOs

Instead of manually listing each DTO type, use assembly scanning to discover all concrete classes in the DTO namespace:

```csharp
Type[] dtoTypes = typeof(DbFooRow).Assembly
    .GetTypes()
    .Where(t => t.Namespace == "Foo.Database.Dtos" &&
                t.IsClass &&
                !t.IsAbstract)
    .ToArray();

DapperMapping.Configure(dtoTypes);
```

**Why assembly scanning:**

- Adding a new DTO doesn't require updating the installer
- Prevents accidental omission of DTO types
- Uses a well-known anchor type (`DbFooRow`) to locate the assembly
- Filters to concrete classes only (no interfaces, no abstract base classes)
    ...
    : IServiceCollectionInstaller
{
    ...
    public void ConfigureServices(IServiceCollection services, IConfiguration configuration)
    {
        // Step 1: Auto-discover all DTO types in Foo.Database.Dtos namespace
        Type[] dtoTypes = typeof(DbFooRow).Assembly
            .GetTypes()
            .Where(t => t.Namespace == "Foo.Database.Dtos" &&
                        t.IsClass &&
...
This brings in the type handlers and `DapperMapping`. Roadbed.Data is already a transitive dependency from Roadbed.Data.Sqlite.

### Step 2: Create DTOs with Column Attributes

```csharp
namespace Foo.Database.Dtos;

using System;
using System.ComponentModel.DataAnnotations.Schema;

/// <summary>
/// Database row DTO for the foo table.
/// </summary>
public sealed class DbFooRow
{
    [Column("id")]
...
The repository uses `SqliteExecutor` from Roadbed.Data.Sqlite and Dapper automatically maps results to the DTO using the `[Column]` attributes.
...
internal sealed class DbFooRepository : BaseClassWithLogging, IDbFooRepository
{
    ...
    public async Task<DbFooRow?> ReadAsync(
        long id,
        CancellationToken cancellationToken = default)
    {
        var request = new DataExecutorRequest(
        ...
        return await SqliteExecutor.QuerySingleOrDefaultAsync<DbFooRow>(
            request,
            this._connectionFactory,
            this._logger,
            cancellationToken);
...
1. Dapper executes the query and gets column names: `id`, `name`, `display_name`, `created_at`, `updated_at`
2. For each column, `DapperMapping` looks for a `[Column]` attribute match on `DbFooRow`
3. `display_name` → matches `[Column("display_name")]` on `DisplayName` property
4. `created_at` → matches `[Column("created_at")]` on `CreatedAt` property
5. The `DapperDateTimeHandler` converts the TEXT value `"2024-01-15 14:30:00"` to a `DateTime` with `DateTimeKind.Utc`

### Step 4: Wire Up the Installer

Add the Dapper configuration to the existing installer (see [Complete Installer Example](architecture-roadbed-dapper.md#complete-installer-example) above).
...
```csharp
// ❌ Wrong — "display_name" column won't match "DisplayName" via [Column]
public sealed class DbFooRow
{
    public string DisplayName { get; set; } = string.Empty;  // No [Column]
}
...
### 4. Manually Listing DTO Types

```csharp
// ❌ Fragile — must update every time a new DTO is added
DapperMapping.Configure(
    typeof(DbFooRow),
    typeof(DbBarRow),
    typeof(DbBazRow));

// ✅ Better — assembly scanning discovers all DTOs automatically
Type[] dtoTypes = typeof(DbFooRow).Assembly
    .GetTypes()
    .Where(t => t.Namespace == "Foo.Database.Dtos" &&
                t.IsClass &&
...
### 5. Assuming DateTime Values Preserve Local Time

```csharp
// ❌ Wrong assumption — Local time is converted to UTC on storage
var entity = new DbFooRow
{
    CreatedAt = new DateTime(2024, 1, 15, 14, 30, 0, DateTimeKind.Local),
};
// Stored as UTC equivalent, e.g., "2024-01-15 20:30:00" (if UTC-6)
...
### 6. Using init-Only Properties on Database DTOs
...
{
    [Column("id")]
    public long Id { get; init; }  // Dapper can't set this
}
...
```csharp
using Roadbed.Data;   // All types (namespace flattened from Roadbed.Data.Dapper)
using Dapper;          // SqlMapper.AddTypeHandler (in installer only)
using System.ComponentModel.DataAnnotations.Schema;  // [Column] attribute (in DTOs only)
```

### Installer Registration Checklist

- [ ] Assembly-scan for all concrete DTO classes in the `.Dtos` namespace
- [ ] Call `DapperMapping.Configure(dtoTypes)` with scanned types
- [ ] Register `DapperDateTimeHandler`
- [ ] Register `DapperNullableDateTimeHandler`
- [ ] Register `DapperDateTimeOffsetHandler`
- [ ] Register `DapperNullableDateTimeOffsetHandler`
